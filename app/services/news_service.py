...
class NewsService:
    ...
    def __init__(self):
        ...
        # Google Search client (required for web enrichment)
    ...
    async def _perform_web_search(
        ...
        # Enrich with Google Custom Search results (general web)
        try:
    ...
    async def _perform_dutch_rss_search(
        ...
        try:
            ...
            # Enrich with Google Custom Search focused on NL domains
            try:
                queries = [f'"{company_name}"']
                if contact_person:
                    queries.append(f'"{company_name}" "{contact_person}"')
    ...
    async def _search_web_content(
        ...
        try:
            ...
            # Enrich with Google Custom Search using the full query
            try:
                google_items = await self.google_search.search(
                    search_query,
                    num=10,
                    lang_nl=True,
                    site_nl_only=False,
                ...
                logger.warning("Google web search failed", error=str(e))
